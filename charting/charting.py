...
class ChartPlotter:
    ...
    def _get_bin_label(self, sentence_length):
        low = (sentence_length // self.bin_size) * self.bin_size
        high = low + self.bin_size - 1
        return f"{low}-{high}"

    def _get_sample_by_sentence(self, sentence, raw_data):
    ...
    def _get_highlighted_text_image_wrapped(self, text, entities):
        """
    ...
    def plot_combined_post_training_charts(self, history, sentences, labels, y_true, y_pred, categories,
        ...
        """
        Generates a figure with multiple subplots including training history, category distributions,
        ...
        TOP_CHART_ROWS = 6
        HIGHLIGHT_ROWS = 4    # For highlighted sample images.
        ...
        for sent in val_sentences:
            info = self._get_sample_by_sentence(sent, raw_data)
        ...
        # --- Confusion Matrices (Remaining Rows) ---
        ...
        # Turn off extra subplots in the confusion matrix area.
